...
- [ ] **FOUND-06**: Bump `engines.node` to `>= 22.3.0` (or `>= 20.6.0` minimum) so `node:test` `t.snapshot()` is stable; fix the broken `npm test` script.
...
- [ ] **INDEX-01**: `list_index_sets` — narrow projection; flags default and writable status
- [ ] **INDEX-02**: `get_index_set` — full DTO with retention/rotation strategies
- [ ] **INDEX-03**: `create_index_set` — supports time-based, size-based, and message-count rotation strategies; supports delete/close/archive retention strategies
- [ ] **INDEX-04**: `update_index_set` — partial-update pattern
...
- [ ] **INDEX-08**: `await_system_job` — poll `/system/jobs/{id}` for async-operation completion (introduced here, reused later)
...
- [ ] **PIPE-01**: `list_pipelines`
- [ ] **PIPE-02**: `get_pipeline`
- [ ] **PIPE-03**: `create_pipeline` — pipeline source (stage definitions referencing rule names)
- [ ] **PIPE-04**: `update_pipeline`
- [ ] **PIPE-05**: `delete_pipeline`
