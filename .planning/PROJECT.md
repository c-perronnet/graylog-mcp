...
---
*Last updated: 2026-05-15 — Phase 3 (Streams & Stream Rules) complete: 12 typed tools shipped (list_streams, get_stream, create_stream, update_stream, delete_stream, start_stream, pause_stream, list_stream_rules, create_stream_rule, update_stream_rule, delete_stream_rule, test_stream_match). C2 stream-cascade footgun mitigated end-to-end via keyed-buckets sha-256 hash over rules+pipeline-connections+event-definitions; ROADMAP SC1 byte-identically proven via two distinct frozen cascade hashes. Cascade-hash helper promoted to `src/tools/_shared/cascade-hash.js` for Phase 4+ reuse. UPDATED D-02 keyed-buckets + D-11 8-variant rule schema (incl. match_input) surfaced during planning. 11/11 STREAM requirements complete; 459/459 tests green; 33 schema-parity assertions across Phases 0+1+2+3. Phase 0 (Foundation): 13/13 FOUND. Phase 1 (Inputs & Extractors): 11/11 INPUT. Phase 2 (Index Sets & Retention): 8/8 INDEX (3 live-Graylog UAT items deferred to /gsd-verify-work).*
