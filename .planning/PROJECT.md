...
<!-- Shipped 2026-05-21. All 4 hypotheses validated; tools registered, tested, and (for entity-sharing + role-management) verified against the live UNESCO Graylog 7.0.6+711d207 instance. -->

- ✓ Agent can grant a user view/manage/own access to a stream/dashboard/search via `share_entity` — wraps `POST /api/authz/shares/entities/{entityGRN}` (the live surface; the brief's `PUT /api/authz/shares/{grn}` was wrong). Read-merge-write, dryRun + sha-256 token + drift refusal, last-`own` guard, structured validation_result/403. — v3.1.0 (Phase 10)
- ✓ A GRN abstraction generalizes the sharing tool to streams + dashboards + saved searches; `GRN_TYPES` pinned to the 6-type milestone set; canonical form locked to 6-token `grn::::<type>:<id>` — v3.1.0 (Phase 8)
- ✓ Agent can read an entity's current grants before mutating them via `get_entity_shares` + `list_grantees` (POST .../prepare with empty `{}` body — verified live) — v3.1.0 (Phase 9)
- ✓ Role management — `list_roles` + `get_role` + `create_role` + `update_role` + `delete_role` + `assign_role` + `unassign_role` against legacy `/api/roles` (selected over `/api/authz/roles` after live recon: 16 built-in roles refused client-side by `assertRoleIsMutable` with `reason: builtin_role_immutable`; full-replace update_role with PITFALL 1 read-merge-write; delete_role cascade preview projects `users_dissociated`; assign_role body=`{}` literal NOT null per Pitfall 3; unassign_role last-admin guard). Catalogue-aware permission validation via `GET /api/system/permissions` with `permitUnknownPermissions: false` opt-out. — v3.1.0 (Phase 11)

### Active

<!-- v3.1.0 milestone-close items deferred from individual phases. -->

- ◻ Throwaway-entity full-apply HUMAN-UAT for `share_entity` against live `test` connection (Phase 10 deferred via Option B; bundled into milestone-close gate)
